...
- **Batch Execution** — `execute_batch_operation()` runs shell commands across
  ...
  timeout protection.
- **Artifact Cleaning** — `clean::execute_batch_clean()` removes build artifacts
  (`target/`, `node_modules/`, etc.) across projects, returning a structured
  `BatchCleanReport` with bytes reclaimed.
- **Analytics** — `stats::generate_analytics_report()` computes per-project disk
  usage, bloat index, and ecosystem totals — filterable by query and tag.
...
- **Shell Execution** — `shell` module provides sandboxed command execution with
